...
let creatNew = (e) => {
    const squreLocation = e.target.id;
    //chack if: player choose from storge & not adding in the air && not adding on top on another element 
    if (flag && myWorld[getIndexRow(squreLocation)][getIndexCol(squreLocation)].classList.length === 1
        && myWorld[getIndexRow(squreLocation) + 1][getIndexCol(squreLocation)].classList.length === 2 /*&& `${blockName}` > 0*/) {
        myWorld[getIndexRow(squreLocation)][getIndexCol(squreLocation)].classList.add(class_Name);
